...
```powershell
mvn -B -ntp verify                 # spotless:check + 单测（H2 内存库，无需 Docker；@Tag("integration") 默认跳过）
mvn -B -ntp verify -Pintegration   # J-07：+ 真 PG 集成（Testcontainers postgres:16-alpine，需 Docker + 本机 uv）
```
...
- 测试用 H2（MODE=PostgreSQL）；集成测试用 Testcontainers 打 `@Tag("integration")`（CI 默认跳过；Schema 经 Alembic `upgrade head` 建真库，非 Hibernate create-drop）
