...
public interface DirectMessageRepository extends JpaRepository<DirectMessageEntity, Long> {
  ...
  @Query(
      "SELECT m FROM DirectMessageEntity m "
          + "WHERE m.status NOT IN ('hidden', 'deleted') AND m.recipientId = :me AND m.senderId <> :me "
  ...
  @Query(
      value =
          "SELECT a.peer_id AS peerId, a.last_message_id AS lastMessageId, "
              ...
              + "           AND m5.status NOT IN ('hidden', 'deleted') AND m5.id > COALESCE("
              ...
              + "    WHERE m.status NOT IN ('hidden', 'deleted') AND (m.sender_id = :me OR m.recipient_id = :me)"
              + "  ) x "
              + "  GROUP BY x.peer_id"
              + ") a "
  ...
  @Query(
      "SELECT COUNT(m) FROM DirectMessageEntity m "
          + "WHERE m.status NOT IN ('hidden', 'deleted') AND m.recipientId = :me AND m.senderId = :peer AND m.id > :sinceId")
  ...
  @Query(
      value =
          "SELECT COUNT(*) FROM direct_messages m "
              + "WHERE m.status NOT IN ('hidden', 'deleted') AND m.recipient_id = :me AND m.id > COALESCE("
              + "  (SELECT r.last_read_id FROM dm_read_state r "
