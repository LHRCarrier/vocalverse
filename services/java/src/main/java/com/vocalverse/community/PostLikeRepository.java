...
public interface PostLikeRepository extends JpaRepository<PostLikeEntity, Long> {
   ...
   *
   * <p>{@code INSERT ... ON CONFLICT DO NOTHING} 由 DB 原子兜底，替代「先查后插」的 check-then-act 竞态窗口（PG 默认 READ
   * COMMITTED 下两次并发 like 会双双查空 → 第二个 INSERT 撞唯一键 → 未捕获 DataIntegrityViolationException →
   * 500）。H2（MODE=PostgreSQL）与 PG 双方言均支持 无目标 {@code ON CONFLICT DO NOTHING}（2026-09-08 实测 H2
   * 2.2.224）。
   */
  ...
  /**
   * 原子删行（J-01）：返回实际删除行数（0/1）—— unlike 只在返回 1（事实行真实存在）时才递减计数， 防并发双击取消重复减计数导致 like_count 与事实行长期漂移。
   */
  @Modifying(clearAutomatically = true, flushAutomatically = true)
  @Query(
