"""show_portrait：关键节点展示角色立绘（**信号工具**：只校验实体 + 发展示信号）。

- 模型在剧情关键时刻调用（角色首次登场 / 重要转折）→ 本工具校验实体（限 npc/pc、未离场）
  → outcome 附带 ``portrait`` 载荷 → ``turn.py`` 转 SSE ``portrait`` 事件 → 前端展示；
- **图源**（docs/56 §4）：读 ``trpg_entities.portrait_media_id``（用户/主持台挂载），
  命中则回带 ``media_id`` + ``url``（``/api/v1/media/{public_id}``）；未挂图 → null，
  前端按实体名命中内置素材（无命中降级占位）；
...
from app.trpg.state import find_entity, portrait_view
...
async def handle(args: ToolArgs, campaign_id: int) -> ToolOutcome:
    ...
    if entity is None:
        return {
            "text": (
                f"本局实体表里还没有角色「{name}」——若刚登场，请先用 enter_character 登记，"
                "或先用叙述让其出场。"
            )
        }
    ...
    view = portrait_view(entity.get("portrait_media_id"))
    if view:
        portrait.update(view)
    return {
