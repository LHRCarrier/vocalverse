...
2. **文档放置检查**：工作结束前逐项核对产出物去向，不得默认丢仓库根：
 ...
 3. ~~**新功能联调测试页（预览机制 · 可删无影响，2026-09-03 固化为强制项）**~~ **【已废弃 · 2026-09-21 组长拍板】**
    - **本项强制要求撤销**：新功能开发**不再要求**提供联调测试页 / 后端 test-only 接口 / 删除清单，PR 审查**不再**以「缺联调测试页」作为退回理由；
    - 历史背景（存档）：曾要求「涉及前后端联动的新功能」在 `apps/web/src/views/preview/` 提供 dev-only 联调页 + 配套后端 test-only 接口（首例 Agent Lab，PR#26），验收后按删除清单移除；
    - 现状：生产即真接口的域（社区 S1、打卡等）早已按 `docs/13 §8` 例外登记免做；本条撤销后**预览画廊机制本身保留**（`views/preview/` + `registry.ts` + `router/preview.ts` 仍供设计稿高保真与视觉验收使用，docs/13 §8 的预览工作流不受影响）；
    - 既有已合入的联调页/开关不追溯删除（按各文件删除清单自行处置）。
