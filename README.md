...
| 本地 ASR 请求 500，uvicorn 日志含 `LocalEntryNotFoundError` / `Please check your internet connection` / httpx 连接错误（httpcore `_sync`） | **whisper 模型缓存未指向仓库 `data/models`**（huggingface 被墙无法在线下载）：应用启动默认注入 `HF_HOME=<仓库>\data\models` + `HF_HUB_OFFLINE=1`（与容器 compose 同约定，docs/06 §8；`scripts/dev-up.ps1` 同样注入）。若手工覆盖过 `HF_HOME` 或更换 ASR 模型，确保模型完整在 `data/models/hub/`（2026-09-04 实测处置见 `worklog/BUG实测/方式B-Python-ASR-HF缓存失配.md`） |
