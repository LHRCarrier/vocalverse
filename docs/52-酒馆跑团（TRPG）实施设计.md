...
**消息翻译（X 式「翻译」按钮，2026-09-21 追加）**：
- 每条 DM 消息**右上角**「翻译」文字按钮：点击 → 服务端翻译（中英互切，方向按内容自动判定）→ 原位展示译文，
  按钮变「原文」；再点切回原文。译文期间不参与逐词高亮（高亮按原文字符偏移）。
- 端点 `POST /api/v1/trpg/translate {text, target?}`（≤2000 字）：扣 llm 桶；失败 → 47003；原文不动、不入库。
- 长按动作条内也有「翻译这条 / 看原文」入口（与角标按钮同一状态）。

**消息长按操作菜单**（替代原「气泡尾重播按钮」）：
- 长按（**420ms**；手指位移 ≤14px 不取消——旧版任何 `touchmove` 都取消，手机上几乎按不出来；
  按住 180ms 出现轻微缩放反馈 + 成功时轻震；桌面右键同效）DM/玩家消息 → 底部动作条：
  **听这句 / 停止朗读**（该条 TTS）、**翻译这条 / 看原文**、**标注这条 / 取消标注**（本地标记，刷新保留）、
  **复制文本**、取消；
- **选中可见性**：动作条头部带所选消息预览（首 48 字），同时该条气泡描边高亮（`.is-active`）——
  解决「长按了不知道选的是哪条」；
- 标注存储：`localStorage` 按剧本分键（`vv_trpg_marks_<campaignId>`，值为内容哈希数组——
  流式行无稳定 message_id，内容哈希可同时覆盖历史重建与流式新增）；
- 气泡/文本节点 `user-select: none` + `-webkit-touch-callout: none` + `touch-action: manipulation`
  （避免原生选择菜单与双击缩放干扰长按；复制走菜单）。

**App 入口**（mobile 惯例：每页顶栏放本页功能；**2026-09-21 组长反馈调整：标题居中 + 功能项可放左**）：
酒馆页左上 `⚙️ 设置` + `⭐ 场景卡`，右上 `📖 切换剧本` + `⚙ 主持台`（后两者仅游玩态）+ `离开`
——游玩态左右 2:3 配平使标题真居中（实测偏移 0px）；无剧本态右侧只剩离开钮，星标回右上（1:2 配平）。
## 11. 已知欠账
