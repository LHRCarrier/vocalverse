...
| Java 严禁进入语音/LLM 热路径（含其 SSE）；**社区域单向 SSE（私信 IM）归 Java**（2026-09-10 收窄 · docs/49 §3.3）；Python 不做管理端 CRUD、不签发 JWT | docs/06 §1 |
...
| R2 Java **不进口**语音/LLM 相关依赖；`/internal/**` 只走服务令牌 | Java 出现 `whisper`/`edge-tts`/`openai` 依赖（**SSE 依赖自 2026-09-10 起允许**：社区域私信长连用 Spring 自带 `SseEmitter`，不引第三方流库 · docs/49 §3.3） |
...
| Java (Spring Boot) | 用户管理、场景/歌曲/LRC/题库/听力素材 CRUD、工单、JWT 签发、（P2）评价看板只读、**社区内容 C 端（2026-09-06 登记：feed/发帖/评论/互动 5 表写方，见 docs/37）**、**社区域单向 SSE（私信 IM 长连推送，2026-09-10 登记 · docs/49 §3）** | **语音/LLM 热路径（含其 SSE）**；指标口径公式；DDL（`ddl-auto=none`）；刷新后写 Python 表；图片/视频上传转码 | → PG（自己表）；← Python 内部 REST |
| 前端 (Vue3) | 页面、录音、SSE 消费、音频队列、埋点上报、报表可视化 | 原生 App；业务状态真源；直连外部 AI | → 网关（唯一出口） |

**边界判据**（新功能落位用）：*该能力是否消耗外部 AI 配额？* 是 → Python；*是否修改账户/内容库/工单且无音频？* 是 → Java；*指标口径定义在哪边？* → Python，Java 只读；*需要长连接/流？* → **看域**：语音/LLM 热路径 → Python；**社区域（无 AI 配额、写方在 Java）→ Java**（2026-09-10 修订，原「一律 Python」按 docs/49 §3.3 收窄）。**「薄服务端」= 不进热路径、不做 DDL、不写指标口径、不吃 AI 配额、不做支付/上传转码**（2026-09-06 修订：原「薄管理端」表述不含社区 C 端，语义按此判据重述）。
...
| 内部委托 | Python→Java | REST `POST /internal/level`、`POST /internal/checkin`（两条，见 docs/21 §4） | `Authorization: Bearer <service-token>` 头（R-17 修订，与代码一致），服务内网直连（**不过网关**） | 3s 超时；失败不阻塞调用方（日志告警 + 幂等键支持重试） |
...
| 练习 | post_likes | **Java**（2026-09-06 反转：点赞对象从打卡变为帖子，键改造 (post_id, liker_id)） | Python（画像）；Java 展示态 | M3 社区（docs/37） |
| 社区 | posts | **Java** | 前端（C 端）；Python 画像（只读） | S1 新表（内容帖 + 打卡卡物化；docs/37） |
| 社区 | post_comments | **Java** | 前端；Python（后续） | S1 新表 |
| 社区 | post_interactions | **Java** | Python（画像）；Java 展示态 | S1 新表（like/coin/share 时序） |
| 社区 | follows | **Java** | Python（后续）；Java | S1 建表、S2 接口 |
...
| ~~Java 侧没有任何场景/歌曲/工单实体~~ | ~~只见 auth 域代码~~ | ✅ **已整改（2026-09-07 提前落地，commit c8cbba2）**：管理端最小集实现（用户管理/内容库 CRUD + LRC 整首重写/题库版本化/工单状态机）；**2026-09-10 该旧管理端 HTTP 面（`/api/v1/admin/**`，27 op）与 `ROLE_ADMIN` 守卫整体退役**，能力迁至独立控制台 `/api/v1/console/**`（`admin_users` 独立身份 + 权限码，删除清单见 docs/50 §15.5）；M-1 角色授权随 0003 落到 `vv_java` |
