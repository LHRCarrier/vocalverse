...
## 5.1 社区主页（2026-09-05 组长拍板 · 与样板并列的已定稿页面）

- `/m/home` 现为**英语社区主页**（非「今日首页」）：X 式顶栏 + 领域文字标签 + 帖子/视频混合流 + 互动行（评论/点赞/投币/分享）；
- 组件拆分与状态规范见 `docs/design-system/vocalverse/pages/community.md`（页面级覆盖优先于 MASTER），结构参考源 = docs/34（Fwitter 粒度，只借结构不搬视觉）；
- 本页沿用 uic 纸面语言（`u-*` 类）；随 §5 收尾统一迁移 token 时并线，**迁移不得改变已批准过的社区页视觉基线**。
