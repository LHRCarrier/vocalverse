...
- **学习页 `MobileLearnView`**（/m/learn，2026-09-05 晚 8：原「练习首页」重建更名并**清空内容**）：
  展示焦点 = **用户学习画像**（组长设计中：展示哪些数据/展示方式/能否「扫一眼明白大量信息」）；
  数据源（M3，docs/02 学习画像演进）：讯飞词级错误/发音分 → 薄弱音素、高频错误词、流利度趋势、场景覆盖度；
  当前 = 画像占位（「建设中」空态）；场景对话/唱吧/自由对话入口 = 学习组底栏，页面不再重复卡；
