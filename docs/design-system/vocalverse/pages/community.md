...
- **练习 Hub `MobilePracticeView`**（/m/practice）：三张入口卡——场景对话（/m/chat）· 自由对话（/m/free-chat）· 唱吧（/m/sing）；
  合并自「口语 + 唱吧」两 tab（docs/14 §12 口语 Hub 思路，复用 u-hub-card 样式）；三页路由不变、进入即二级页（dock 贴底）；
