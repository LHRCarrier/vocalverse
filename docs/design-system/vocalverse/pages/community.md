...
- **「我的」/m/me 已舍弃（2026-09-09 组长拍板）**：信息收敛进账户抽屉（`MobileAccountDrawer`）——菜单 = 我的学习（→ /m/learn）/ 消息 / 设置与隐私（演示帧 toast，M3 接真实设置页）/ 退出登录；原 MobileMeView.vue 已删除（统计/目标/设置列表随弃，学习目标 M3 再议）；
