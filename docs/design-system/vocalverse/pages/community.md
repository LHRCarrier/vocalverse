...
- **学习页 `MobileLearnView`**（/m/learn，2026-09-05 晚 8 重建更名；晚 9 组长拍板：**参考 Duolingo——等级制度 + 经验条为画像主要焦点**；
  2026-09-09 画像焦点改造 = docs/36 信息架构；**v2 组长反馈：卡片太多像拼凑 → 合并 3 卡（参考社区页帖子卡密度）+ 顶栏去头像 + 热力图精简，待组长验收**）：
  - **顶栏**：学习页 `showAvatar=false`（画像页 LV 徽章已是身份焦点，头像冗余；MobileTopBar 新增 prop 默认 true，其他页不变）；
  - **卡 1 · 画像主卡**（身份行 ｜ 分割线 ｜ 今日四数，仿帖子卡作者行+正文+互动行）：
    - 身份行：LV 黑徽章（84px 金星等级名）+ 金色 XP 经验条 + `70/500 XP · 距下一级` + 连续天数 🔥 + 本周时长 ⏱；
    - 今日四数行（border-top 弱分割 · 4 列竖细分隔线）：今日练习 / 今日分钟 / 本周练习 / 总 XP——每卡一数一标签（DuoDash StatCard 式）；
  - **卡 2 · 学习热力图**（主焦点 · 精简版）：仅 12 周 × 7 天网格（`repeat(12,1fr)` **填满卡宽，无右空**）+ 右下角「+75 XP」；**无图例、无「少/多」、无近 12 周/全年切换、无日期前缀**；色阶 = 无练习 track / lv1 pop-green-soft / lv2 pop-green / lv3 success；今天 = accent 蓝 outline；选中 = inner 墨描边；点格 → 右下角经验更新；未来日期置灰禁用；
  - **卡 3 · 能力画像**（高密度一大卡）：「能力画像」标题 → 流利度趋势（近 7 天柱图 + 近 7 天均分徽章，绿阶三档 ≥85 深绿 / ≥78 绿 / 其余浅绿 + 今日 accent 蓝）→ 场景掌握度（8 场景一条强度条：≥80 绿 / ≥60 黄 / <60 红 + 分数）→ 结论区（border-top 弱分割：薄弱音素 / 高频错误词 chips + AI 一句话点评 wave 图标块）；
  - **经验规则（演示）**：场景对话回合评分 +15 XP；自由对话回合结束 +5 XP（`progress` store：localStorage 持久化，等级表 LV1 英语新手 0 / LV2 口语学徒 100 / LV3 对话能手 250 / LV4 表达达人 500 / LV5 流利大师 900；M3 后端加权结算）；
  - **LV 联动面（同一份 level）**：我的页档案卡（LV·等级名 + 距下一级 XP）· 账户抽屉（@username · LVx）· 社区帖子卡（`LV4` meta）· 私信列表/会话头（对方 `LVx` 徽章）· 学习页画像卡——「完成练习加经验 → 全 app 展示 LV」闭环；
...
- **统一顶栏 `MobileTopBar`**（每个移动页面都有）：**grid 三列 `1fr auto 1fr`（标题真居中，不受左右宽度影响）**；
  左 = **全局头像（固定最左侧第一位，组长定规：左侧不可有其他图标，点击开账户抽屉）** + 离开钮（tabler **logout 镜像**：门+箭头朝左，34px 与头像同级；学习组功能页点击 → 回学习主页，报告页 → router.back()）；
