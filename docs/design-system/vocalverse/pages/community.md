...
- **「我的」/m/me 已舍弃（2026-09-09 组长拍板）**：信息收敛进账户抽屉（`MobileAccountDrawer`）——菜单 = 我的学习（→ /m/learn）/ 通知（→ /m/notifications）/ 设置与隐私（**抽屉内展开子面板**：帮助与反馈/数据与隐私/关于声语界，演示帧 toast，M3 接真实设置页）/ 退出登录；
- **通知中心 `/m/notifications`（2026-09-09 消息收敛）**：X 式 tab 分流——私信（原 /m/messages 会话列表收敛）+ 通知（互动通知：点赞/评论/关注/系统，未读蓝点 + 底色双通道）；抽屉/社区底栏/社区页写消息三入口统一指向；/m/messages 路由保留（会话详情承接链路）；M3 接埋点事件派生通知流；
