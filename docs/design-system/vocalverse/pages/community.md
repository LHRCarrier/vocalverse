...
- **「我的」/m/me 已舍弃（2026-09-09 组长拍板）**：信息收敛进账户抽屉（`MobileAccountDrawer`）——菜单 = 我的学习（**抽屉内展开四模块**：我的单词/社区足迹/我的发音/练习情况 → /m/learn/:module，点击子项收起）/ 通知（→ /m/notifications）/ 设置与隐私（**抽屉内展开子面板**：帮助与反馈/数据与隐私/关于声语界，演示帧 toast，M3 接真实设置页）/ 退出登录；展开项 chevron 展开时旋转 180°（下拉指示与功能一致）；
