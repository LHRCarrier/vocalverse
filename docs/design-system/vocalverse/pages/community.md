...
## 导航（2026-09-05 组长拍板 2：底部「我的」→「私信」）

- **底部 tab**：首页 / 口语 / ＋（中央主钮）/ 唱吧 / **私信**（原「我的」移除）；
- **「我的」（/m/me）入口** = 首页顶栏头像 → 抽屉「你的资料」；「设置与隐私」→ /m/me（我的页含设置区块）；
- 抽屉「消息」→ /m/messages；抽屉底部红色**退出登录**（auth.clear → /login）；
- **私信页**：`MobileMessagesView`（会话列表：头像/名字/最后消息/时间/未读点蓝）+ `MobileChatView`（气泡会话：我方炭黑右/对方灰左 + 时间戳 + 输入条；发送 1.2s 后对方演示自动回复轮换）；演示帧，M3 接真实消息流；
