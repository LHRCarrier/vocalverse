...
## 导航（2026-09-05 组长拍板 5：5 位对称 TabBar + 练习 Hub）

- **5 位对称（X/Grok 同构）**：社区 / 搜索 / **＋发帖（中央第 3 位，严格居中，左右各 2）** / 练习 / 私信；

| 显示 | 隐藏（二级页） |
|---|---|
| `/m/home` 社区 · `/m/search` 搜索 · `/m/practice` 练习 · `/m/messages` 私信 | 会话 · 报告 · 我的 · 口语场景/自由对话 · 唱吧 · 发帖页（从 ＋ 进入） |

- **练习首页 `MobilePracticeView`**（/m/practice，与社区 home 同级设计语言）：顶栏（练习 + 报告入口）→
  **今日目标卡**（线稿锚点 + 主 CTA「开始练习」→ /m/chat）→ 统计行（连续/累计/平均，M3 学习画像替换）→
  **场景对话·推荐**（fetchScenarios 真实接口 top3，失败 fallback 演示场景，L 徽章 + 描述）→ AI 自由对话卡 → **唱吧精选暗卡**（每屏唯一深色卡：本周精选 + 幽灵「去跟唱」）；场景对话/自由对话/唱吧 三页路由不变、进入即二级页（dock 贴底）；
