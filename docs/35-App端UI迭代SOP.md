...
| 2 | **头像固定最左侧第一位**（2026-09-21 组长反馈修订：头像之后可放「左侧功能项」） | 顶栏 = 头像（36px，永远最左）→ **左侧功能项**（`left` 插槽，可选）→ 标题（grid 三列）→ 右侧 actions + 离开钮；**标题居中规则**：三列均为内容宽度驱动，左右两组宽度差的一半 = 标题偏移量 → 按钮多的页面用 left 插槽配平（例：酒馆游玩态左 2 右 3，实测偏移 0px） |
| 3 | **离开钮 = tabler `logout` scaleX(-1) 镜像**（门+箭头朝左） | 34px 与头像同级；**项目无 ← 语言**；语义 = 离开当前页（学习组功能页 → /m/learn；报告页 router.back()） |
| 4 | **底栏双场景分组** | 社区组（🏠/🔍/＋📝/📚学习出口/✉️）· 学习组（🏠出口/☕/📖中央/🎵/💬）；5 位对称（左右各 2 + 中央正位）；沉浸页（/m/compose、**/m/tavern**）无全局底栏；新增页面归属写 TabBar `group` computed |
...
- **沉浸页**：/m/compose（发帖）、/m/post/:postId（帖子详情，社区 S3 2026-09-09）、/m/me/profile（我的资料，社区 S3）、/m/reader/:chapterId（阅读器）及读书域页、**/m/tavern（酒馆，2026-09-22 起页内自带「大堂/酒馆跑团/角色卡/纪事」四项导航，出口 = 顶栏离开钮）**；其余一切页面属于社区组或学习组。
