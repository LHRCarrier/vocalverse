...
| 模型其实在本机 | `data/models/hub/models--Systran--faster-whisper-small` **完整存在**（model.bin 483MB + tokenizer + vocab，宿主预下载，容器挂载约定 docs/06 §8） |
| 但本地进程找不到 | 本地 uvicorn **无 `HF_HOME`/`HF_HUB_OFFLINE`**（容器由 compose 注入；方式 B 没有任何等价注入）→ huggingface_hub 1.29 走默认缓存 `%USERPROFILE%\.cache\huggingface`（里面只有 Qwen，无 whisper） |
| 联网下载失败 | huggingface.co **被墙**（本机 SSL 握手即失败；同仓库 compose 注释明示「容器访问 huggingface 被墙」）→ `snapshot_download` 抛 `LocalEntryNotFoundError`（链出 httpcore **同步** httpx `ConnectError`） |
...
1. **`services/python/app/main.py`**：进程入口处 `os.environ.setdefault` HF 三件套——`HF_HOME=<仓库>/data/models`、`HF_HUB_OFFLINE=1`、`HF_HUB_DISABLE_XET=1`（与容器 compose 同约定；必须在任何 huggingface_hub 导入之前；用户进程已显式设置时尊重之）。
2. **`scripts/dev-up.ps1`**：启动 Python 前显式注入同款三变量（与容器参数逐一对齐，日志可见、可查）。
...
4. **容器有、本地没有的环境变量 = 隐性契约缺口**：compose 已把 HF 三件套钉死（offline + 本地缓存卷），方式 B 文档没提 → 每次本地联调必踩。修复应落在**代码默认值**（`setdefault` 尊重显式覆盖），而不是再写一行文档让每个人记。
